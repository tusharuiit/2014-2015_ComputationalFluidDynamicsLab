...
#endif // _ARBITRARYGEOMETRY_
    {
        ...
        for(i = 0; i < PARAMQ; i++)
            for(x = 0; x < (local_xlength[0] + 2) * (local_xlength[1] + 2) * (local_xlength[2] + 2); x++)
                collideField[x + (local_xlength[0] + 2) * (local_xlength[1] + 2) * (local_xlength[2] + 2) * i] = streamField[x + (local_xlength[0] + 2) * (local_xlength[1] + 2) * (local_xlength[2] + 2) * i] = LATTICEWEIGHTS[i];
